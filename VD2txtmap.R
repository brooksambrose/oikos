...
)
{
	...
	require("RSQLite")
	con <- dbConnect(drv="SQLite", dbname=paste(vdl,"/Voice_Dream_Reader.sqlite",sep="")) ## connect to db
...
for(i in citekeys){
	...
	if(clust.lab) {
		require(fpc)
		pk<-c(1,diff(as.numeric(factor(pamk(grph$ZLOCATION[subheads|greens])$pamobject$clustering+grph$span[subheads|greens]))))
	}
    ...
    text(x=0-(rhs*.0125/3),y=apply(cbind(ytop,ybottom),1,mean),grph$page[samp]-1,col="white",srt=90,cex=.5,offset=0,adj=c(.5,.25))
    ...
    text(x=rhs+(rhs*.0125/3),y=apply(cbind(ytop,ybottom),1,mean),round(grph$ZLOCATION[samp]/tlen*100,0),col="white",srt=270,cex=.5,offset=0,adj=c(.5,.25))
	...
	if(sum(grph$sc)) text(x=apply(grph[!!grph$sc,c("x1","x2")],1,mean),y=t$y[heads[heads|subheads]][grph$span[!!grph$sc]]+.6,labels=rep("!",sum(grph$sc)),col="red",cex=.5,font=2)
...
if(F){
rm(list=ls())
vdl="/Users/bambrose/Dropbox/2012-2013/2012-2013_C_Spring/proposal/VoiceDreamDB/Jess's iPad Mini/Voice Dream Library" ## delete this
citekeys="Hargens1988ub"
i=citekeys
debug=T
graphics.off()
}
if(F){
source("/Users/bambrose/Dropbox/2012-2013/2012-2013_C_Spring/proposal/VoiceDreamDB/VD2txtmap_rewrite_2014-9.R")
VDtextmap(
	vdl="/Users/bambrose/Dropbox/2012-2013/2012-2013_C_Spring/proposal/VoiceDreamDB/Jess's iPad Mini/Voice Dream Library"
	,citekeys="menu"
	,clust.lab=T
	)
}
